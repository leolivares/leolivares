...
<?php
	include 'consultas/consultap_stats_1.php';
	include 'consultas/consultap_stats_2.php';
	include 'consultas/consultap_stats_3.php';
?>
...
<script>
  ...
  for (var i = new_array.length-1; i >= 0; i--) { 
  	console.log(i);
	  dates[i] = new_array[i][1].split(" ")[0].split("-").slice(0, 2).reverse().join("-");
	  money[i+1] = new_array[i][0];
  }
...
<script>
	...
	var x = new Array(products.length);
	for (var i = x.length - 1; i >= 0; i--) { 
	  x[i] = new Array(products[i][0], products[i][1]);
	}
	console.log(x);
