...
Persona = namedtuple("Persona", ["nombre", "apellido",
                                 "nombre_completo", "caracteristicas", "adn"])
...
def crear_persona(codigo):
    ...
    tipos, caracteristicas = obtener_caracteristicas(letras, numeros)
    adn = obtener_adn(letras, tipos)

    return Persona(nombre, apellido, nombre_completo, caracteristicas, adn)


def obtener_info(codigo):
    generador = funcion_generadora(codigo)
    numeros = [next(generador) for i in range(11)]
    letras = [x for x in codigo if not x.isdigit()]
    return numeros, letras


def obtener_nombre(letras, numeros):
    nombre = "".join(letras[:numeros[0]])
    apellido = "".join(letras[numeros[0]:numeros[0]+numeros[1]])
    return nombre, apellido


def obtener_caracteristicas(letras, numeros):
    letras = "".join(letras[numeros[0] + numeros[1]:])
    tipos = [letras[i:i + 3] for i in range(0, len(letras), 3)]
    conjunto = list(zip(tipos, numeros[2:]))
    caracteristicas = {k: v for k, v in conjunto}
    return tipos, caracteristicas


def obtener_adn(letras, tipos):
    letras = "".join(tipos[9:])
    adn = [letras[i:i + 3] for i in range(0, len(letras), 3)]
    return adn
...
def analisis(person):
    datos = {caract: analizar_caracteristica(caract, person)
             for caract in person.caracteristicas}
    return datos
...
def analizar_caracteristica(caract, person):
    lista = listas[person.caracteristicas[caract]]
    if lista != [""]:
        seleccionados = [person.adn[int(k)] for k in lista]
        set = {verificar(k, person) for k in seleccionados}
...
def valor_caracteristica(tag_identificador, persona):
    ...
    if len(person) != 0:
        analisis = {caract: analizar_caracteristica(caract, person[0])
                    for caract in person[0].caracteristicas}
        valores = {caract: determinar_fenotipo(dicc, caract)
